...
export function DashboardPage() {
  const { stats, weeklyVolume, upcoming, loading, error, monthlyVolume } = useDashboard();
    ...
    <div className="p-6 md:p-10 space-y-8 pb-24 md:pb-10">
        ...
        <StatCard
          ...
          sub={
            <div className="flex items-center justify-between w-full text-text-secondary">
              ...
              {stats?.total ? <span className="font-semibold text-text-primary">{stats.total} total</span> : null}
            </div>
          }
        />
        <StatCard
          ...
          sub={
            <span className="text-text-secondary">Agendados para esta semana</span>
          }
        />
        <StatCard
          ...
          sub={
            <div className="flex justify-between w-full text-text-secondary">
              ...
              {stats?.cancelledCount ? (
                <span className="text-error font-medium">{stats.cancelledCount} Cancelados</span>
              ) : null}
            </div>
          }
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        <div className="md:col-span-2" style={{ height: '380px' }}>
          <PatientVolumeChart weeklyData={weeklyVolume} monthlyData={monthlyVolume} />
        </div>
        <div>
